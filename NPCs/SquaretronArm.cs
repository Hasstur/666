...
{
	public class SquaretronArm : ModNPC
	{
		...
		{
			if (Main.npc[(int)npc.ai[0]].type == mod.NPCType("SquaretronArm2") && Main.npc[(int)npc.ai[0]].active)
			{
				npc.Center = CenterPoint(Main.npc[(int)npc.ai[3]].Center, Main.npc[(int)npc.ai[0]].Center);
